classdef AArm
    ...
    methods
        function self = AArm(tip_point, node1, node2)
            % Constructor function. Takes 2 lines and 2 nodes as arguments

            self.tip = tip_point;
            tip_loc = tip_point.location;
            endpoints_loc = [node1.location, node2.location];
            self.endpoints = [node1, node2];
            self.static_plane = Plane(tip_point.location, node1.location, node2.location);
